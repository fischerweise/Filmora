...
class SearchViewController: UIViewController{
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(SearchMoviePagerTableViewCell.self)
        tableView.register(SearchMovieTableViewCell.self)
        return tableView
    }()
...
extension SearchViewController {
    func setupUI() {
        ...
        search?.searchBar.placeholder = "Type a movie here to search"
...
extension SearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == searchTableView{
            ...
            if section == 0 {
                let cell: SearchMoviePagerTableViewCell = self.tableView.dequeueReusableCell(for: indexPath)
                cell.loadCell(movies: presenter?.cellNowPlayingMovie())
                cell.delegate = self
                return cell
            } else  {
                let cell: SearchMovieTableViewCell = self.tableView.dequeueReusableCell(for: indexPath)
